...
在聊天输入框使用 `/goal <目标>` 启动目标模式,例如 `/goal 完成登录模块重构并运行测试`。目标会持久化到会话,可用以下命令控制:

- `/goal status` 查看目标与预算
- `/goal pause` / `/goal resume` 暂停或恢复
- `/goal edit <目标>` 修改目标
- `/goal complete` / `/goal blocked` 结束目标
- `/goal clear` 清除目标

目标处于 `active` 时,模型完成一轮后会在同一次运行中收到一次内部 continuation 并继续工作,不会用固定的 8 次上限截断长任务。目标模式可在每个会话输入框上方的目标栏独立开关,开启后普通 Agent 提示会自动创建持久目标。目标完成、暂停、阻塞、达到显式 token 预算、供应商或运行时出错、供应商用量受限或检测到重复无进展时停止。连续 5 次供应商或运行时错误会自动暂停目标,成功推进或手动恢复会清零错误计数。运行中的新提示和目标控制命令默认加入队列,输入框上方会显示目标、TodoWrite 步骤,并提供暂停、恢复和删除控制;队列项可由用户显式打断当前运行并立即执行。目标运行会自动使用 Agent 工具模式。
