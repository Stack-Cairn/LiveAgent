...
async function fetchTunnelState() {
  return callBackend<TunnelStateSnapshot>("tunnel_state", {});
}
...
async function executeTunnelManager(
  ...
  try {
    ...
    if (action === "create") {
      ...
      await callBackend<void>("tunnel_create", { input });
      const snapshot = await fetchTunnelState();
    ...
    if (action === "close") {
      ...
      await callBackend<void>("tunnel_close", { tunnel_id: id });
    ...
    // 再取样碰运气）。返回后快照一定是新的。
    await callBackend<void>("tunnel_check", { tunnel_id: id });
    const snapshot = await fetchTunnelState();
