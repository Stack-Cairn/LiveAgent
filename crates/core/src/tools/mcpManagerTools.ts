...
async function stopRuntime(serverId: string, warnings: string[]) {
  try {
    const stopped = await callBackend<McpStopServerResponse>("mcp_stop_server", {
      server_id: serverId,
    });
...
async function runtimeStatus(serverId: string) {
  return callBackend<McpRuntimeStatus>("mcp_runtime_status", { server_id: serverId });
}
...
async function runtimeTest(
  ...
  return callBackend<McpRuntimeTestResponse>(restart ? "mcp_restart_server" : "mcp_test_server", {
    server,
