...
export async function ensureBuiltinSkills() {
  try {
    return await callBackend<SystemBuiltinSkillSeedResponse[]>("system_ensure_builtin_skills");
  } catch (error) {
...
async function maybeAttachReadmeFallbackInline(skill: SkillSummary): Promise<SkillSummary> {
  ...
  try {
    const metadata = await callBackend<SystemReadSkillMetadataResponse>("system_read_skill_metadata", {
...
export async function readSkillText(params: {
  ...
  return callBackend<SystemReadSkillTextResponse>("system_read_skill_text", {
    path: params.path,
...
export async function manageSkill(
  ...
  const response = await callBackend<SystemManageSkillResponse>("system_manage_skill", {
    payload: params,
  } as any);
