...
async function runHook(
  ...
  if (hook.type === "command") {
    ...
    if (!script) return;
    await callBackend("hook_run_script", {
  ...
  const response = await callBackend<HookHttpRunResponse>("hook_run_http_requests", {
    requests,
    scope_id: scopeId,
  });
  if (!response.ok) {
...
export function createHookRunScope(params: {
  ...
  return {
    ...
    cancel: () => {
      ...
      accepting = false;
      cancelled = true;
      void callBackend("hook_cancel_scope", { scope_id: scopeId }).catch(() => undefined);
    },
  };
}
