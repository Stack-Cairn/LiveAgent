...
/**
 * Persistence port for the subagent store. The HTTP-to-backend implementation is the
...
export function createBackendSubagentStoreIpc(): SubagentStoreIpc {
  ...
  return {
    upsertIdentity: async (input) =>
      stripNulls(await callBackend<SubagentIdentity>("subagent_identity_upsert", { input })),
    listIdentities: async (input) =>
      (await callBackend<SubagentIdentity[]>("subagent_identity_list", { input })).map(stripNulls),
    saveRun: (input) =>
      enqueueRunWrite(input.run.id, async () => {
        await callBackend("subagent_run_save", { input });
      }),
    listRuns: async (input) =>
      (await callBackend<SubagentRunSummary[]>("subagent_run_list", { input })).map(stripNulls),
    loadRun: async (id) => {
      const record = await callBackend<SubagentRunStateRecord | null>("subagent_run_load", {
        input: { id },
      });
    ...
    pruneRuns: (input) => callBackend<SubagentPruneResult>("subagent_run_prune", { input }),
    appendMessage: async (input) =>
      stripNulls(await callBackend<SubagentMessageRecord>("subagent_message_append", { input })),
    listMessages: async (input) =>
      (await callBackend<SubagentMessageRecord[]>("subagent_message_list", { input })).map(stripNulls),
  };
}

export const backendSubagentStoreIpc = createBackendSubagentStoreIpc();
