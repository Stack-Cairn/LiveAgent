...
const validateModule = loader.loadModule(coreSrc("chat/compaction/validate.ts"));
...
test("verification signals are extracted from recent payload messages", () => {
    ...
    [
      {
        ...
        toolCallId: "t",
        isError: false,
        content: "compiled crates/frontend/src/lib/chat/compaction/policy.ts",
      },
    ],
  ...
  assert.ok(
    signals.some((signal) => signal.includes("crates/frontend/src/lib/chat/compaction/policy.ts")),
  );
});
