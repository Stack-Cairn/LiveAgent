...
//! 拉起监听。整个模块暂时留着，因为 backend 还没接手 `/image-proxy` 与
...
async fn handle_image_proxy(Query(query): Query<ImageProxyQuery>, headers: HeaderMap) -> Response {
    ...
    let client = match backend::services::system_proxy::cached_client() {
        Ok(client) => client,
        Err(error) => {
...
async fn handle_proxy(
    ...
    let client = if use_system_proxy {
        match backend::services::system_proxy::cached_client() {
            Ok(client) => client,
            Err(error) => {
                return error_response(
                    StatusCode::BAD_GATEWAY,
