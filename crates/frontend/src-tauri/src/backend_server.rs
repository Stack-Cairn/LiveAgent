...
pub struct BackendServer {
    ...
    pub engine: std::sync::Arc<tokio::sync::Mutex<Option<backend::engine_process::EngineProcess>>>,
}
...
pub async fn start_backend_server(engine_bundle: Option<std::path::PathBuf>) -> Result<BackendServer, String> {
    ...
    // 生成密码和内部 token。
    let password = backend::auth::generate_password();
    let internal_token = backend::auth::generate_password();

    // 生成认证配置。
    let auth = std::sync::Arc::new(backend::auth::AuthConfig::new(password.clone(), internal_token.clone()));
