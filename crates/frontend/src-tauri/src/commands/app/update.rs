...
fn github_client() -> Result<reqwest::Client, String> {
    ...
    backend::services::system_proxy::client_builder_with_os_proxy_fallback()?
...
fn build_updater(
    ...
    if let Some(proxy_url) = backend::services::system_proxy::current_proxy_url()? {
...
pub fn app_restart(app: AppHandle) -> Result<(), String> {
    ...
    if let Some(registry) = app
        .try_state::<std::sync::Arc<backend::runtime::managed_process::ManagedProcessRegistry>>()
    {
        registry.shutdown_cleanup();
    }
    if let Some(power) = app
        .try_state::<std::sync::Arc<backend::services::power_activity::PowerActivityManager>>()
    {
        power.clear_all();
    }
