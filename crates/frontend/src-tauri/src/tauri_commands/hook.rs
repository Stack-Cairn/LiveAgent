...
use backend::commands::hook::*;
use backend::runtime::shell_runner::{
    run_shell_script_with_envs, ShellCancelFlag, ShellCancelToken, ShellRunResponse,
};
use backend::runtime::task_runner::{
    build_http_client, resolve_workdir, run_single_http_request, HttpRequestInput,
};
use backend::services::automation::validate::{MAX_HOOK_TIMEOUT_MS, MIN_HOOK_TIMEOUT_MS};
use serde::Serialize;
...
pub async fn hook_run_script(
    ...
    backend::commands::hook::hook_run_script(
        workdir,
        script,
        timeout_ms,
        scope_id,
...
pub async fn hook_run_http_requests(
    ...
    backend::commands::hook::hook_run_http_requests(requests, scope_id, registry.inner()).await
}
...
pub async fn hook_cancel_scope(
    ...
    backend::commands::hook::hook_cancel_scope(scope_id, registry.inner()).await
}
