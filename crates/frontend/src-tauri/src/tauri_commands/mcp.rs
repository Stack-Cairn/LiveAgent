...
use backend::commands::mcp::*;
use backend::runtime::platform::{
...
pub async fn mcp_list_tools(
    ...
    backend::commands::mcp::mcp_list_tools(state.inner(), servers).await
}
...
pub async fn mcp_call_tool(
    ...
    backend::commands::mcp::mcp_call_tool(
        state.inner(),
        run_registry.inner(),
        server_id,
        tool_name,
...
pub async fn mcp_runtime_status(
    ...
    backend::commands::mcp::mcp_runtime_status(state.inner(), server_id).await
}
...
pub async fn mcp_stop_server(
    ...
    backend::commands::mcp::mcp_stop_server(state.inner(), server_id).await
}
...
pub async fn mcp_test_server(
    ...
    backend::commands::mcp::mcp_test_server(state.inner(), server, include_schema, persist).await
}
...
pub async fn mcp_restart_server(
    ...
    backend::commands::mcp::mcp_restart_server(state.inner(), server, include_schema, persist)
        .await
}
