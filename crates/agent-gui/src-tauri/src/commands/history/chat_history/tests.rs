...
mod tests {
    ...
    #[test]
    fn codex_converter_maps_messages_tools_and_title() {
        let temp = tempfile::tempdir().expect("create temp dir");
        let path = temp.path().join("rollout-test.jsonl");
        std::fs::write(
            &path,
            concat!(
                r#"{"type":"session_meta","timestamp":"2026-01-01T00:00:00Z","payload":{"id":"session-1","session_id":"session-1","cwd":"/tmp/project"}}"#,
                "\n",
        ...
        assert_eq!(skipped, 1);
        assert_eq!(conversation.id, "codex:session-1");
        assert_eq!(conversation.title, "Imported title");
        assert_eq!(conversation.model, "gpt-5.4");
        assert_eq!(conversation.cwd.as_deref(), Some("/tmp/project"));
        assert_eq!(conversation.messages.len(), 5);
        assert_eq!(conversation.messages[0]["role"], json!("user"));
        assert_eq!(conversation.messages[1]["content"][0]["type"], json!("thinking"));
        assert_eq!(conversation.messages[2]["content"][0]["name"], json!("shell"));
        assert_eq!(conversation.messages[3]["toolCallId"], json!("call-1"));
        assert_eq!(conversation.messages[4]["content"][0]["text"], json!("Done"));
    }
