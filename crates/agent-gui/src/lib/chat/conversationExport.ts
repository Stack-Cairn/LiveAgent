...
function pushContextBlocksFromRound(
  ...
  // Attach round.meta.usage at most once so estimateContextUsage does not pick a
  // trailing usage-only placeholder as the anchor (which would skip tool results).
  let usageAttached = false;
  const takeUsage = () => {
    if (usageAttached) return undefined;
    usageAttached = true;
    return round.meta?.usage;
  };
  for (const block of blocks) {
    ...
    if (block.kind === "tool") {
      ...
      if (toolResult && typeof toolResult === "object") {
        // Flush toolCall fragment before the tool result. Do NOT attach usage
        // here: multi-tool rounds share one meta.usage for the whole response;
        // early attachment would leave later toolCalls after the anchor and
        // double-count them in estimateContextUsage.
        if (content.length > 0) {
          out.push({
            role: "assistant",
  ...
  } else if (!usageAttached && round.meta?.usage) {
