...
use agent_backend::tunnel::TunnelDataPlane;
use agent_core::services::tunnel::{TunnelBinding, TunnelDataPlane as _};
...
async fn tunnels_are_restored_after_a_restart_with_fresh_ports() {
    ...
    use agent_core::services::tunnel::{TunnelCreateInput, TunnelStore};

    // 隔离 HOME：TunnelStore 落库到 settings 库，不能碰开发者真实数据。
    let dir = std::env::temp_dir().join(format!("agent-backend-tunnel-restart-{}", std::process::id()));
    ...
    let first = Arc::new(TunnelStore::new(
        ...
        Arc::clone(&first_plane) as Arc<dyn agent_core::services::tunnel::TunnelDataPlane>,
    ));
    first
    ...
    let second = Arc::new(TunnelStore::new(
        Arc::new(EventBus::new()),
        Arc::clone(&second_plane) as Arc<dyn agent_core::services::tunnel::TunnelDataPlane>,
    ));
