...
import { getGatewayOrigin, setGatewayOrigin } from "@/lib/gatewayOrigin";
import {
...
function useDashboardAuth() {
  ...
  const [loginGatewayUrl, setLoginGatewayUrl] = useState(() => getGatewayOrigin());
  ...
  const submit = () => {
    ...
    setAuthError(null);
    try {
      setGatewayOrigin(loginGatewayUrl);
    } catch (error) {
      setAuthError(asErrorMessage(error, "Gateway 地址无效。"));
      setAuthSubmitting(false);
      return;
    }
  ...
  if (!token) {
    return (
      <LoginPage
        ...
        onGatewayUrlChange={(nextGatewayUrl) => {
          setLoginGatewayUrl(nextGatewayUrl);
          if (authError) {
            setAuthError(null);
          }
        }}
