...
export async function listAdminAgents(
  ...
  const url = new URL(`${getGatewayOrigin()}/api/agents`);
...
export async function issueAdminToken(
  ...
  const url = `${getGatewayOrigin()}/api/agents/${encodeURIComponent(agentId)}/token`;
...
export async function updateAdminAgentName(
  ...
  const url = `${getGatewayOrigin()}/api/agents/${encodeURIComponent(agentId)}`;
  const response = await fetch(url, {
    method: "PATCH",
    headers: { ...authHeaders(token), "Content-Type": "application/json" },
    body: JSON.stringify({ name: name.trim() }),
