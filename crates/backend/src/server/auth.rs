...
///
/// 原先住在 `engine_proxy`（唯一的消费者是那条只许内部服务调的事件回流路由）。
/// 那个模块随 Node 引擎一起没了，身份判定本就是认证的产物，搬回这里。
...
impl AuthConfig {
    ...
    pub fn verify_with_identity(&self, presented: &str) -> Option<CallerIdentity> {
        ...
        if password_match {
            return Some(CallerIdentity::User);
        }
