//! 上传暂存 / 附件读取 / 调试日志落盘。
//!
//! 从桌面壳 `commands/app/system.rs` 下沉：这些命令是纯文件系统操作，
//! 没有任何 tauri 依赖，core（Node 引擎）经 HTTP 调用它们，桌面壳的
//! Tauri 薄包装也直接转发到这里——同一份实现，两个调用面。
...
#[derive(Debug, Clone)]
pub struct SystemReadableFileUploadInput {
    pub file_name: String,
...
pub fn canonicalize_upload_workdir(workdir: &str) -> Result<PathBuf, String> {
...
pub fn system_import_readable_file_paths_sync(
    workdir: String,
    paths: Vec<String>,
...
pub fn import_readable_file_paths_into_workdir(
    workdir: &Path,
...
pub fn system_import_uploaded_readable_files_sync(
    workdir: String,
...
pub fn system_import_uploaded_readable_files_from_base64_sync(
    workdir: String,
...
pub fn system_read_uploaded_image_preview_sync(
    workdir: String,
...
pub fn system_read_uploaded_native_attachment_sync(
    workdir: String,
...
pub fn system_append_debug_jsonl_sync(conversation_id: String, entry: Value) -> Result<(), String> {
...
/// HTTP 路由直连的异步命令面（generate-routes.mjs 的 EXTRA_COMMANDS 指到这里）。
/// 与桌面壳的 Tauri 薄包装完全同构：spawn_blocking + 同步实现。
