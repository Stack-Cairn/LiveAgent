...
4. 模型列表：按 `group` 折叠分组，行内能力图标、接口标签（自动浅色 / 显式实色）或"生图"标记、厂商实现标签、连通性测试、编辑与移除；标题行有文档、收起、搜索、过滤、"全部测试"、"刷新模型列表"、"手动添加"，分组可整组删除。
5. 更多设置：流式重试、Prompt 缓存、原生搜索、系统代理。
6. 用量查询。
...
**抽屉二：编辑模型。** 六节：标识（模型 ID、远端 ID、显示名、系列、模型类型 对话 / 生图——生图模型不进聊天选择器，由 `generate_image` 工具调用，见 6.7）；目录（一行摘要：分区 / 条目、快照、发布、状态、价格，"查看目录"跳目录浏览器）；能力与模态（一张表：目录值 / 有效值 / 覆盖，目录值列带候选标记，目录与供应商声明冲突时可一键采纳供应商值，见 6.2）；限额（同样三列，另有请求参数 temperature / maxTokens / topP 的模型级覆盖，见 6.3）；思考档位只读（来自目录）；路由（接口、厂商实现、凭据、缓存提示协议，其下折叠"实际请求预览"与"故障转移候选"）。每个覆盖项显示来源与"还原"。
