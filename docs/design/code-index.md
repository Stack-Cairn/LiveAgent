...
- **单 FTS 表（unicode61）+ CJK bigram 预切分**，不建 trigram 双表：代码主体是
  ASCII 标识符，unicode61 把 `_`/`-` 当分隔符恰好让 `foo bar` 命中 `foo_bar`。
  但 unicode61 对连续 CJK 是单 token——纯中文查询在原始文本上永远撞不上词边界
  （评审 #630）。v3 起写入与查询两侧对 CJK 连续段做同源 bigram 展开
  （`store::segment_cjk_for_fts`，"监听失效" → "监听 听失 失效"；单字查询词走
  FTS5 前缀匹配 `"锁"*`），中文/假名/谚文获得词法路能力，语义路继续兜概念查询。
  （trigram 双表方案针对纯中文正文，代码场景体积翻倍不值；bigram 预切分只膨胀
  CJK 文本，对 ASCII 代码零成本。）
- **语义路相关性阈值**（`search::SEMANTIC_MAX_DISTANCE`，单位向量 L2 距离
  0.72 ≈ 余弦 0.74）：vec0 KNN 无条件返回 k 个最近邻，无意义查询也会凑满
  top-k；超阈候选按噪声丢弃，两路全空时如实返回零结果。前提是 embedding
  落库前显式 L2 归一化（`embedder::l2_normalize`，对 e5 幂等兜底）。
- **生成物内容探测**（`walker::looks_generated`）：扩展名/大小拦不住的生成物
  （1.1 万行 Cython 产物、protobuf/Go generate 输出、minified 漏网）按头部
  标记（`@generated`/`DO NOT EDIT`/`Generated by Cython` 等，前 10 行）与
  超长行（前 5 行 > 4 KB）识别，不入索引、存量清除。Grep 仍可兜底检索它们。
...
- FTS 查询词经 memory-index 同款引号转义，防 MATCH 语法注入；查询先过与
  写入侧同源的 CJK bigram 切分，单字 CJK 词走前缀匹配（`"锁"*`）。
- 语义路候选按 `SEMANTIC_MAX_DISTANCE` 过滤后再进 RRF（见 §2 取舍）。
- 返回 top-N（默认 8，上限 20）：`path:start-end` + `symbol` + 现读片段
  （单块截断 ~1200 字符），附 RRF 分与来源（lexical/semantic/both）。
- 响应携带两个可观测性字段（工具层拼进给模型的文本，评审 #630 条目 1/6）：
  - `degraded?`：语义路不可用（模型下载中/失败/超时）降级纯词法时的说明；
  - `indexing?`：索引构建进行中（含阶段与 N/M 进度）或索引为空时的说明——
    构建期检索合法但结果可能不完整、排序随索引增长漂移，模型据此决定
    Grep 兜底或稍后重试，而不是把局部结果当全量事实。
