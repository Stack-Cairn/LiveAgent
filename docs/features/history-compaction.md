...
Desktop 的会话更多菜单还提供“在新窗口中打开”。每个会话最多对应一个独立窗口；重复打开时聚焦已有窗口，主窗口选择该会话时也会转而聚焦独立窗口，避免同一会话出现两个可编辑视图。正在运行或切换中的会话不允许弹出，独立窗口关闭后可重新在主窗口打开。独立窗口复用 Chat 页面，但不重复注册托盘、全局快捷键、Gateway bridge、自动化和更新检查等应用级职责。
