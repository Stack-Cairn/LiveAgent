...
| 层级 | 主要路径 | 技术栈 | 核心职责 |
|---|---|---|---|
| 共享应用 UI | `crates/agent-ui/src` | React、TypeScript、Tailwind | GUI/WebUI/Mobile 共用的 Settings、Skills Hub、MCP Hub、聊天侧边栏、输入栏、项目工具和领域逻辑。 |
...
| 移动端 Mobile | `crates/agent-mobile` | Tauri 2、React、TypeScript、Vite、WebView | Android 客户端，复用 WebUI 的 Gateway 数据控制器与共享 UI，经 Tauri 外壳 + shims 远程操作本地 Agent。 |
...
| Android Mobile | `crates/agent-mobile/src/main.tsx`、`src/App.tsx` | Gateway `/ws/v2`、`/api/*`（经 shims 重定向） | Android WebView 客户端，仅持有 token 与脱敏设置，不本地执行工具。 |
...
| 多端可用性对齐 | GUI/WebUI/Mobile 三端从 `crates/agent-ui` 复用组件与领域逻辑，通过 `@liveagent/adapters` 提供 Tauri 或 Gateway 适配器；应用独有能力由扩展注册表选择性启用。 |
